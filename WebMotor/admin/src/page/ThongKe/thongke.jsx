  ...
  const fetchDoanhThuNam = async () => {
    try {
      ...
      const tongDoanhThuNam = danhSach
        .filter(lich => new Date(lich.ngay_dat_hang).getFullYear() === namHienTai && lich.trang_thai === 2)
    ...
    try {
      ...
      const tongDoanhThuThang = danhSachLichKham
        .filter(lich => {
          ...
          return ngaymua.getFullYear() === nam && ngaymua.getMonth() === thang && lich.trang_thai === 2;
        })
        .reduce((tong, lich) => tong + (lich.tong_tien || 0), 0);
